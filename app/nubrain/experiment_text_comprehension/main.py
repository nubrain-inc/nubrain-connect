...
from nubrain.experiment_text_comprehension.comprehension_questions import (
    run_comprehension_questions,
)
...
def experiment_text_comprehension(config: dict):
    ...
    # the participant can close their eyes / rest. These are rest cues only (not
    ...
    try:
        ...
        for word in text:
            ...
            if io.wait_until(t_stim_end_expected):
                running = False
                break
            ...
            if stimulus_block_counter == stimuli_per_block:
                ...
                # The onset of the current inter-block interval is also the end time of
                ...
                # Pump the event queue (via io.wait_until), so we can quit during
                # rest.
            ...
            # End of this stimulus presentation. Display ISI grey screen.
            screen.fill(text_config.rest_condition_color)
            pygame.display.flip()
        ...
        # Only run the questions if the participant has not already quit during the
        # reading phase.
        if running:
            qa_result = run_comprehension_questions(
                io=io,
                screen=screen,
                ...
                correct_answer_duration_ms=3000,
            )
            n_correct_answers = qa_result["n_correct"]
            response_log = qa_result["response_log"]
            if qa_result["quit_requested"]:
                running = False
        else:
            n_correct_answers = 0
            response_log = []
